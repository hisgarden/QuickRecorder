...
class AdvancedRecordingTests: XCTestCase {
    ...
    // MARK: - Audio Mixing Tests
    
    func testAudioMixing_DeviceSelection() throws {
        // Given - Audio device configuration
        let audioSettings = recordEngine.getAudioSettings()
        let micDevice = settingsManager.micDevice
        
        // When - Validate audio setup
        let hasAudioConfig = !audioSettings.isEmpty
        let hasMicConfig = !micDevice.isEmpty
        
        // Then - Audio configuration should be valid
        XCTAssertTrue(hasAudioConfig)
        XCTAssertTrue(hasMicConfig)
    }
    
    func testAudioMixing_QualitySettings() throws {
        // Given - Audio quality options
        let originalAudioQuality = settingsManager.audioQuality
        let qualityOptions = ["low", "mid", "high"]
        
        // When/Then - Test audio quality settings
        for quality in qualityOptions {
            settingsManager.audioQuality = quality
            XCTAssertEqual(settingsManager.audioQuality, quality)
        }
        
        // Cleanup
        settingsManager.audioQuality = originalAudioQuality
    }
    
    func testAudioMixing_FormatConfiguration() throws {
        // Given - Audio format options
        let originalFormat = settingsManager.audioFormat
        let formats = [AudioFormat.aac, AudioFormat.mp3]
        
        // When/Then - Test audio formats
