    ...
    var body: some View {
        NavigationView {
            List(selection: $selectedItem) {
                ...
                NavigationLink(
                    destination: RecorderView(), tag: "Recorder", selection: $selectedItem
                ) {
                ...
                NavigationLink(
                    destination: BlocklistView(), tag: "Blaoklist", selection: $selectedItem
                ) {
        ...
        SForm(spacing: 10) {
            SGroupBox(label: "Recorder") {
                ...
                if #available(macOS 14, *) {
                    SSteper(
                        "Presenter Overlay Delay", value: $poSafeDelay, min: 0, max: 99,
                        tips:
                            "If enabling Presenter Overlay causes recording failure, please increase this value."
                    )
                    SDivider()
                }
            ...
            SGroupBox {
                ...
                SToggle(
                    "Hide Control Center Icons", isOn: $hideCCenter,
                    tips:
                        "Hide the clock, Wi-Fi, bluetooth, volume and other system icons in the menu bar."
                )
                SDivider()
                SToggle(
                    "Highlight the Mouse Cursor", isOn: $highlightMouse,
                    tips: "Not available for \"Single Window Capture\"")
                SDivider()
                SToggle(
                    "Exclude Files on Desktop", isOn: $hideDesktopFiles,
                    tips:
                        "If enabled, all files on the Desktop will be hidden from the video when recording."
                )
            }
...
extension AppDelegate {
    ...
    @objc func setLoginItem(_ sender: NSMenuItem) {
        ...
        do {
            ...
            print(
                "Failed to \(sender.state == .on ? "enable" : "disable") launch at login: \(error.localizedDescription)"
            )
        }
    }
}
