    ...
    var body: some View {
        ZStack(alignment: .top) {
            ...
            VStack(spacing: 3) {
                Button {
                    ...
                    PlayerSlider(
                        percentage: $audioPlayerManager.progress,
                        audioLength: $audioPlayerManager.audioLength
                    ) { editing in
                        if !editing {
                            let newTime =
                                audioPlayerManager.progress * audioPlayerManager.getPlayerDuration()
                ...
                HStack(spacing: 4) {
                    ...
                    Button {
                        ...
                        ZStack {
                            ...
                            Image(
                                systemName: audioPlayerManager.isPlaying
                ...
                Button {
                    ...
                    HStack(spacing: 14) {
                        HStack(spacing: 2) {
                            ...
                            Text("\(Int(audioPlayerManager.sysVol * 100))%").foregroundColor(
                                .secondary
                            ).frame(width: 40)
                        ...
                        HStack(spacing: 2) {
                            ...
                            Text("\(Int(audioPlayerManager.micVol * 100))%").foregroundColor(
                                .secondary
                            ).frame(width: 40)
        ...
        .onAppear {
            audioPlayerManager.loadAudioFiles(
                format: document.info.format, package: fileURL, encoder: document.info.encoder,
        ...
        GeometryReader { geometry in
            ZStack(alignment: .leading) {
                    ...
                    Rectangle()
                        ...
                        .frame(
                            width: geometry.size.width
                                * CGFloat(min(1.0, self.percentage / maxValue)))
                ...
                Circle()
                    ...
                    .offset(
                        x: geometry.size.width * CGFloat(min(1.0, self.percentage / maxValue)) - 8)
                ...
                .gesture(
                    DragGesture(minimumDistance: 0)
                        .onChanged { value in
                            self.percentage = min(
                                max(0, Float(value.location.x / geometry.size.width) * maxValue),
                                maxValue)
    ...
    var body: some View {
        GeometryReader { geometry in
            VStack(spacing: 2) {
                HStack {
                    Text(
                        "\(String(format: "%.2d:%.2d:%.2d", isDragging ? Int(temporaryPercentage * audioLength) / 3600 : Int(percentage * audioLength) / 3600, isDragging ? Int(temporaryPercentage * audioLength) / 60 : Int(percentage * audioLength) / 60, isDragging ? Int(temporaryPercentage * audioLength) % 60 : Int(percentage * audioLength) % 60))"
                    ).foregroundColor(.secondary)
                    Spacer()
                    Text(
                        "\(String(format: "%.2d:%.2d:%.2d", Int(audioLength) / 3600, Int(audioLength) / 60, Int(audioLength) % 60))"
                    )
                    .foregroundColor(.secondary)
                }
                ZStack(alignment: .leading) {
                                ...
                                width: geometry.size.width
                                    * CGFloat(
                                        min(
                                            1.0,
                                            self.isDragging
                                                ? self.temporaryPercentage : self.percentage)))
                                ...
                                x: geometry.size.width
                                    * CGFloat(
                                        min(
                                            1.0,
                                            self.isDragging
                                                ? self.temporaryPercentage : self.percentage)) - 3)
                    }
                    Rectangle()
                        .cornerRadius(12)
                        .foregroundColor(.primary)
                        ...
                        .offset(
                            x: geometry.size.width
                                * CGFloat(
                                    min(
                                        1.0,
                                        self.isDragging ? self.temporaryPercentage : self.percentage
                                    )) - 2)
                }.onHover { hovering in isHover = hovering }
                    .compositingGroup()
                    .gesture(
                        DragGesture(minimumDistance: 0)
                            .onChanged { value in
                                ...
                                self.temporaryPercentage = min(
                                    max(0, Double(value.location.x / geometry.size.width)), 1)
                                self.isDragging = true  // Indicate dragging
                                self.onEditingChanged(true)  // Notify that editing started
                            }
                            .onEnded { value in
...
struct qmaPackageHandle: FileDocument {
    ...
    init(
        info: Info = Info(
            format: "m4a", encoder: "aac", exportMP3: false, sysVol: 1.0, micVol: 1.0),
        sysAudio: Data = Data(), micAudio: Data = Data()
    ) {
        self.info = info
    ...
    init(configuration: ReadConfiguration) throws {
        ...
        guard let sysAudioFileWrapper = fileWrappers["sys.\(info.format)"],
            let sysAudio = sysAudioFileWrapper.regularFileContents
        else {
            throw CocoaError(.fileReadCorruptFile)
        }
        ...
        guard let micAudioFileWrapper = fileWrappers["mic.\(info.format)"],
            let micAudio = micAudioFileWrapper.regularFileContents
        else {
            throw CocoaError(.fileReadCorruptFile)
        }
...
class AudioPlayerManager: ObservableObject {
    ...
    func loadAudioFiles(format: String, package: URL, encoder: String, saveMP3: Bool) {
        do {
            ...
            exportMP3 = saveMP3
            packageURL = package
            audioFile1 = try AVAudioFile(
                forReading: package.appendingPathComponent("sys.\(format)"))
            audioFile2 = try AVAudioFile(
                forReading: package.appendingPathComponent("mic.\(format)"))

            audioLength =
                Double(audioFile1?.length ?? 0)
                / (audioFile1?.processingFormat.sampleRate ?? 48000.0)
    ...
    func seek(to time: Double) {
        ...
        if frameCount > 0 {
            ...
            playerNode1.scheduleSegment(
                audioFile1, startingFrame: startFrame, frameCount: frameCount, at: nil,
                completionHandler: nil)
            playerNode2.scheduleSegment(
                audioFile2, startingFrame: startFrame, frameCount: frameCount, at: nil,
                completionHandler: nil)
    ...
    private func startProgressTimer() {
        timer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in
            if let lastRenderTime = self.playerNode1.lastRenderTime,
                let playerTime = self.playerNode1.playerTime(forNodeTime: lastRenderTime)
            {
                let currentTime =
                    Double(self.lastStartFramePosition + playerTime.sampleTime)
    ...
    func export() {
        ...
        showSavePanel(
            defaultFileName:
                "\(packageURL.deletingPathExtension().appendingPathExtension(format).lastPathComponent)",
            exportMP3: exportMP3
        ) { url, saveAsMP3 in
    ...
    func saveFile(_ url: URL, saveAsMP3: Bool = false) {
        ...
        if url.pathExtension != self.fileFormat {
            url = url.appendingPathExtension(self.fileFormat)
        }
        ...
        Thread.detachNewThread {
            ...
            do {
                guard let audioFile1 = self.audioFile1, let audioFile2 = self.audioFile2 else {
                    return
                }
                ...
                let outputFile = try AVAudioFile(
                    forWriting: url, settings: audioSettings, commonFormat: .pcmFormatFloat32,
                    interleaved: false)
                self.engine.stop()
                try self.engine.enableManualRenderingMode(
                    .offline, format: outputFormat, maximumFrameCount: 4096)
                ...
                while self.engine.manualRenderingSampleTime < duration {
                    let framesToRender = min(
                        UInt32(buffer.frameCapacity),
                        UInt32(duration - self.engine.manualRenderingSampleTime))
                ...
                let body = String(
                    format: "File saved to: %@".local, url.path.removingPercentEncoding!)
                let id = "quickrecorder.completed.\(UUID().uuidString)"

                // MP3 export removed - use AAC, ALAC, FLAC, or Opus instead

                SCContext.showNotification(title: title, body: body, id: id)
            } catch {
                SCContext.showNotification(
                    title: "Failed to save file".local, body: "\(error.localizedDescription)",
    ...
    private func showSavePanel(
        defaultFileName: String, exportMP3: Bool, completion: @escaping (URL?, Bool) -> Void
    ) {
        ...
        let checkBox = NSButton(
            checkboxWithTitle: "Export as MP3".local, target: self,
            action: #selector(checkBoxToggled(_:)))
        checkBox.state = exportMP3 ? .on : .off

        let accessoryView = NSView(
            frame: NSRect(x: 0, y: 0, width: checkBox.frame.width, height: checkBox.frame.height))
