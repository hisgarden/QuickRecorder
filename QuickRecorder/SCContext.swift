...
class SCContext {
    ...
    static func getSelf() -> SCRunningApplication? {
        guard let content = availableContent else { 
            print("Warning: availableContent is nil in getSelf()")
            return nil 
        }
        return content.applications.first(where: { Bundle.main.bundleIdentifier == $0.bundleIdentifier })
    }
    
    static func getSelfWindows() -> [SCWindow]? {
        guard let content = availableContent else { 
            print("Warning: availableContent is nil in getSelfWindows()")
            return nil 
        }
    ...
    static func getApps(hideSelf: Bool = true) -> [SCRunningApplication] {
        guard let content = availableContent else {
            print("Warning: availableContent is nil in getApps(), attempting to update...")
            if let freshContent = updateAvailableContentSync() {
                availableContent = freshContent
                return getApps(hideSelf: hideSelf)
            } else {
                print("Failed to get available content for apps - returning empty list")
                return []
            }
        }
    ...
    static func getWindows(isOnScreen: Bool = true, hideSelf: Bool = true) -> [SCWindow] {
        ...
        guard let content = availableContent else {
            print("Warning: availableContent is nil, attempting to update content...")
            // Try to get content synchronously if it's nil
            if let freshContent = updateAvailableContentSync() {
                availableContent = freshContent
                // Recursive call with updated content
                return getWindows(isOnScreen: isOnScreen, hideSelf: hideSelf)
            } else {
                print("Failed to get available content - returning empty window list")
                return windows
            }
        }
    ...
    static func getSaveDirectory(capture: Bool = false) -> String {
        let directory = ud.string(forKey: "saveDirectory") ?? NSSearchPathForDirectoriesInDomains(.moviesDirectory, .userDomainMask, true).first ?? NSHomeDirectory()
    ...
    static func checkScreenRecordingPermission() async -> Bool {
        // Use a simple approach: try to get content and handle the response
        // If we get content, permissions are granted
        // If we get userDeclined error, permissions are not granted  
        // If we get other errors, assume permissions issue
        return await withCheckedContinuation { continuation in
            SCShareableContent.getExcludingDesktopWindows(false, onScreenWindowsOnly: true) { content, error in
                if let error = error {
                    if let scError = error as? SCStreamError, scError.code == .userDeclined {
                        // User explicitly declined - don't update availableContent
                    ...
                    // Successfully got content - permissions are granted
                    availableContent = content
    ...
    static func requestScreenRecordingPermissionIfNeeded() async -> Bool {
        // First, check if we already have permissions by trying to get content
        // This will only show the system dialog if permissions haven't been granted yet
        let hasPermission = await checkScreenRecordingPermission()
        if hasPermission {
            // We already have permissions, proceed with recording
            return true
        }
        
        // If we don't have permissions, the system dialog was already shown by checkScreenRecordingPermission()
        // Now wait a moment and check again to see if user granted permissions
        try? await Task.sleep(nanoseconds: 500_000_000) // Wait 0.5 seconds
        
        let permissionGranted = await checkScreenRecordingPermission()
        if !permissionGranted {
            // User did not grant permissions, show our custom informational dialog
            await MainActor.run {
                let alert = NSAlert()
                alert.messageText = "Screen Recording Permission Required"
                alert.informativeText = "QuickRecorder needs screen recording permission to capture screen content. Please grant permission in System Settings > Privacy & Security > Screen Recording and try again."
